...
Псевдоклассы - это атрибуты,
назначаемые строго к селекторам
с намерением определить реакцию
...
`:focus`- при получение фокуса элемента
...
`:first-clild`- выбор первого дочернего элемента

`:last-child` - выбор последнего дочернего элемента

`:nth-child(n)` - выбор n-ого дочернего элемента 

### Добавление стилей к дочерним элементам

При наведении на блок с классом
«parent» у дочернего элемента
с классом «child» цвет фона
изменится на синий.
